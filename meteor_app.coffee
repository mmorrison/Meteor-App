Villains = new Meteor.Collection("villains")

if Meteor.is_client
  
  #window.villians = Villians
  
  window.villainsInsert = (key,value) ->
    #Villians.insert({name: key, description: value})
  
  Template.cards.villains = () ->
    return Villains.find({}, {sort: {likes: -1, name: 1}})
  
  Template.hello.greeting = () ->
    return "Vote For Your Favorite Super Villain"
    
  Template.villain.events =
    'click .btn-primary' : (e) ->
      e.preventDefault()
      if typeof console isnt 'undefined'
        Villains.update({_id: this._id}, {$inc: {likes: 1}});
...
if Meteor.is_server
  Meteor.startup () ->
    ...
    if Villains.find().count() is 0
      ...
      for villain, i of villains
        Villains.insert({name: i[0].name, description: i[0].description, likes: i[0].likes, img_url: i[0].img})